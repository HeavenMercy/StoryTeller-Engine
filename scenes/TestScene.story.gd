...
func _init(listener: st.StoryListener = null).():
	...
	listener.load_commands([
		st.StoryCommandNamespace.new(st.NAMESPACE_DEFAULT, default_activate) \
	...
	teller = st.StoryTeller.new(listener) \
		.load_story_chunks([
			...
			st.StoryChunk.new('intro', """
				The story teller says 'Hello' to you!
			...
			""") \
				...
				# .in_namespace(st.NAMESPACE_DEFAULT),

			st.StoryChunk.new('said_hi', """
				[color=gray]You reply to the storyteller.[/color]
				He smiles to  the ears like a freaking maniac.
				'Welcome to my new world', he says
			""").end(),

			st.StoryChunk.new('ignored', """
				[color=gray]You ignore the storyteller[/color]
